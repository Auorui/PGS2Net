# https://github.com/GlassyWu/AECR-Net/blob/main/models/AECRNet.py
import torch
    ...
    def forward(self, x):
        ...
        if self.training and (not frozen):
            ...
            if self.groups == 1:
                ...
                Id = torch.eye(self.num_features, dtype=X.dtype, device=X.device)
                Id = Id.unsqueeze(0).expand(self.groups, -1, -1)
                Cov = torch.baddbmm(
                    Id,
                    X.transpose(1, 2),
                    X,
...
class AECRNet(nn.Module):
    def __init__(self, input_nc=3, output_nc=3, ngf=64):
        ...
        self.dcn_block = DeformConv(256, 256)
