    ...
    def __init__(self, channel, reduction=8, bias=False):
        ...
        if channel == 48:
            ...
            self.conv3 = nn.Sequential(
                nn.ConvTranspose2d(channel * 2, channel, kernel_size=3, stride=2, padding=1, output_padding=1),
                nn.ReLU(inplace=False))
            ...
            self.conv2 = nn.Sequential(
                ...
                nn.ReLU(inplace=False))
            self.conv3 = nn.Sequential(
                nn.ConvTranspose2d(channel * 4, channel, kernel_size=5, stride=4, padding=1, output_padding=1),
                nn.ReLU(inplace=False))
...
class MABDT(nn.Module):
    def __init__(self, in_channel=3, out_channel=4, dim=24, window_size=8,
                 ...
                 use_checkpoint=False, token_projection='conv', token_mlp='ffn', se_layer=False, img_size=256,
                 **kwargs):
        super(MABDT, self).__init__()
        # 编码层的层数
