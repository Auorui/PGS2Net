...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        #    f"Input image size ({H}*{W}) doesn't match trinitynet ({self.img_size[0]}*{self.img_size[1]})."
...
class TrinityNet(nn.Module):
    def __init__(self, in_chans=3, out_chans=3, img_size=[256, 256], depths=[8, 8, 8, 8],
                 embed_dim=32, ape=False, patch_norm=True, mlp_ratio=4.0, qkv_bias=True, qk_scale=8,
                 num_heads=[8, 8, 8, 8], window_size=8, drop_rate=0, drop_path_rate=0.1,
                 attn_drop_rate=0, use_checkpoint=False):
        super(TrinityNet, self).__init__()
        self.img_size = img_size
        self.in_chans = in_chans
...
TRAINING:
  ...
  TRAIN_PS: [256, 256]
  VAL_PS: [256, 256]
  TRAIN_DIR: './data/train_data' # path to training data
  VAL_DIR: './data/val_data' # path to validation data
...
TEST:
  ...
  TEST_PS: [256, 256]
  TEST_DIR: './data/test_data' # path to validation data
  MODE: 'FR' #FR: PSNR, SSIM; NR: UIQM, UCIQE; NFR: both;
