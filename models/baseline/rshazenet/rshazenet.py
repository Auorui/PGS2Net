...
class Downsample(nn.Module):
    def __init__(self, n_feat):
        ...
        self.body = nn.Sequential(
            nn.Conv2d(n_feat, n_feat // 2, kernel_size=to_2tuple(3), padding=1, padding_mode='reflect', bias=False),
            nn.PixelUnshuffle(2))
...
class BasicBlock(nn.Module):
    def __init__(self, dim, division_ratio=4):
        ...
        self.conv_3 = nn.Conv2d(self.dim_partial, self.dim_partial, kernel_size=to_2tuple(3), padding=3, dilation=3,
                                groups=self.dim_partial)
        self.conv_5 = nn.Conv2d(self.dim_partial, self.dim_partial, kernel_size=to_2tuple(5), padding=6, dilation=3,
                                groups=self.dim_partial)
        self.conv_7 = nn.Conv2d(self.dim_partial, self.dim_partial, kernel_size=to_2tuple(7), padding=9, dilation=3,
                                groups=self.dim_partial)
    ...
    def forward(self, x):
        input_ = x
        x_1, x_2, x_3, x_4 = torch.split(x, [self.dim_partial, self.dim_partial, self.dim_partial, self.dim_partial],
                                         dim=1)
        x_1 = self.conv_1(x_1)
        x_2 = self.conv_3(x_2)
        x_3 = self.conv_5(x_3)
...
class RSHazeNet(nn.Module):
    def __init__(self, in_chans=3, out_chans=4, dim=32, depths=(2, 3, 4)):
        ...
        self.skip_connection_level_2_pre = nn.Sequential(
            *[BasicBlock(int(dim * 2 ** 1)) for _ in range(depths[1] // 3)])
        ...
        self.skip_connection_level_3_pre = nn.Sequential(
            *[BasicBlock(int(dim * 2 ** 2)) for _ in range(depths[2] // 2)])
        ...
        self.output_level_1 = nn.Conv2d(dim, out_chans, kernel_size=to_2tuple(3), padding=1, padding_mode='reflect',
                                        bias=False)
